...
// Simple 32x32 canvas-sprites (procedural pixel art)
const SPRITE_SIZE=32;let frame=0;
function drawPlayerSprite(x,y,dir){ctx.save();ctx.translate(x*TILE+4,y*TILE+4);ctx.scale(1,1);
  // body
  ...
  // sword swing hint
  if(frame%20<10 && dir==='right'){ctx.fillStyle='#ddd';ctx.fillRect(26,14,6,2);} 
  if(frame%20<10 && dir==='left'){ctx.fillStyle='#ddd';ctx.fillRect(0,14,6,2);} 
  if(frame%20<10 && dir==='up'){ctx.fillStyle='#ddd';ctx.fillRect(14,0,2,6);} 
  if(frame%20<10 && dir==='down'){ctx.fillStyle='#ddd';ctx.fillRect(14,26,2,6);} 
ctx.restore();}
...
function move(dx,dy,dir){player.dir=dir;const nx=player.x+dx,ny=player.y+dy;if(canMove(nx,ny)){player.x=nx;player.y=ny;}
  const room=rooms[roomIndex];if(room.exit&&player.x===room.exit.x&&player.y===0){roomIndex=Math.min(roomIndex+1,rooms.length-1);player.x=5;player.y=9;}
  draw();}
