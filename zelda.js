...
function canMove(nx,ny){const room=rooms[roomIndex];if(nx<0||ny<0||nx>=10||ny>=10)return false;for(const w of room.walls){if(w.y===0&&room.exit&&nx===room.exit.x&&ny===0)continue;if(nx>=w.x&&nx<w.x+w.w&&ny>=w.y&&ny<w.y+w.h)return false;}return true;}
...
bindTouch('up',()=>move(0,-1));bindTouch('down',()=>move(0,1));bindTouch('left',()=>move(-1,0));bindTouch('right',()=>move(1,0));bindTouch('btnA',attack);draw();